    ...
    def __init__(self,
        ...
        if save_name is None:
            save_name = name + ' ' + model.name + ' E' + str(epochs) + ' lr' + str(
                train_setting['optimizer'].defaults['lr']) + ' decay' + str(train_setting['lr_decay']) + ' tau' + str(tau)
        ...
        if client_num is None and client_list is not None:
            ...
            client_list = [Client(i, copy.deepcopy(
                model), device, train_setting, metric_list, tau) for i in range(client_num)]
...
class Client(fp.Client):
    ...
    def train(self, epochs):
        ...
        for idx, param in enumerate(model_params):
            param.requires_grad = True if idx == len(
                model_params) - 1 else False
        ...
        model_params = list(self.per_model.parameters())
        for idx, param in enumerate(model_params):
            param.requires_grad = False if idx == len(
                model_params) - 1 else True
